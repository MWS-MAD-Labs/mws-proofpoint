  ...
  const handleSave = async () => {
    ...
    try {
      if (qType === "SCALE") {
        ...
        if (!score || isNaN(num)) { setSaving(false); return; }
...
export default function ObservationsPage() {
  ...
  const handleAcknowledge = async () => {
    ...
    try {
      ...
      showAlert("success", "Observation acknowledged. Manager has been notified.");
      await fetchObservations();
      await loadDetail(selected.id);
    } catch {
      showAlert("error", "A network error occurred.");
  ...
  const canAcknowledge =
    ...
    (String(selected?.staffId) === String(currentUser?.id) || isAdmin);
                  ...
                  {observations.map((obs) => (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        {obs.acknowledgedAt && (isAdmin || isManager || isDirector) && (
                ...
                <div className="px-6 py-4 border-b border-border/50">
                  ...
                  {showAcknowledgementInfo && (
                    <div className="mt-2 text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2 flex items-center gap-1.5">
                      ...
                      Staff <strong>{fullName(selected.staff)}</strong> has acknowledged this observation on {formatDate(selected.acknowledgedAt)}.
                      This observation is now complete.
                    </div>
                  )}
                  ...
                  {!canEdit && selected.status === "acknowledged" && (
                    <div className="mt-2 text-xs text-emerald-600 bg-emerald-50 border border-emerald-100 rounded-lg px-3 py-1.5 flex items-center gap-1.5">
                      <CheckCircle2 className="w-3.5 h-3.5 flex-shrink-0" />
                      You have acknowledged this observation on {formatDate(selected.acknowledgedAt)}.
                    </div>
                  )}
                </div>
