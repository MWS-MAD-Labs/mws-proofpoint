...
// ✅ FIX: merge duplicate react imports + duplicate utils imports jadi satu
...
// ✅ FIX: cn dan getAutomaticPeriod digabung dalam satu import
import { cn, getAutomaticPeriod } from '@/lib/utils';
import {
  ClipboardList,
  Save,
  Send,
...
function AssessmentContent() {
  ...
  useEffect(() => {
    if (!assessmentId) setPeriod(getAutomaticPeriod());
  }, [assessmentId]);
  ...
  useEffect(() => {
    const fetchAssignedRubric = async () => {
      ...
      if (error) { console.error('Failed to fetch department roles:', error); return; }
      if (data) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const configs = data as any[];
        const normalizedRoles = roles.map(r => r.toLowerCase());
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        let assignedConfig: any = null;
        ...
        for (const role of normalizedRoles) {
          ...
          if (match) { assignedConfig = match; break; }
        }
        if (!assignedConfig) {
          for (const role of normalizedRoles) {
            const match = configs.find(c => !c.department_id && c.role.toLowerCase() === role);
            if (match) { assignedConfig = match; break; }
          }
        }
        if (!assignedConfig) {
    ...
    if (!assessmentId && profile) fetchAssignedRubric();
  ...
  const handleCreate = async () => {
    ...
    const existingDraft = assessments.find((a: Assessment) =>
      a.template_id === selectedTemplate && a.period === period && a.status !== 'acknowledged'
    );
    if (existingDraft) { router.push(`/assessment?id=${existingDraft.id}`); return; }
    setIsCreating(true);
    const newAssessment = await createAssessment(selectedTemplate, period);
    if (newAssessment) router.push(`/assessment?id=${newAssessment.id}`);
    setIsCreating(false);
  };
                          ...
                          <SelectItem key={t.id} value={t.id} className="focus:bg-primary focus:text-white">
                            <div className="flex flex-col py-1">
                              ...
                              <span className="text-[10px] opacity-70">
                                {t.template_type === 'KPI_APPRAISAL' ? 'KPI Appraisal'
                                  : t.template_type === 'CLASSROOM_OBSERVATION' ? 'Classroom Observation'
                                  : t.template_type === 'GENERIC' ? 'General Framework'
              ...
              <div className="flex flex-col gap-4 pt-2">
                ...
                >
                  {isCreating ? (
                    ...
                    <><ClipboardList className="h-4 w-4 mr-2" />Continue Assessment</>
                  ) : (
                    <><Send className="h-4 w-4 mr-2" />Start Performance Review</>
                  )}
                </Button>
              </div>
            </div>
    ...
    <div className="max-w-7xl mx-auto py-8">
      {isReturned && assessment?.return_feedback && (
        <Alert className="mb-8 border-2 shadow-lg animate-in fade-in slide-in-from-top-4 duration-500 bg-amber-50 border-amber-500/50">
          ...
          <AlertTitle className="font-bold text-lg mb-2 text-amber-800">Assessment Returned for Revision</AlertTitle>
          <AlertDescription className="space-y-3">
            <p className="text-amber-900">Your assessment has been returned by your reviewer. Please review the feedback below and make the necessary corrections before resubmitting.</p>
            <div className="bg-white/50 border border-amber-200 rounded-lg p-4 mt-3">
              ...
              <p className="text-amber-950 whitespace-pre-wrap leading-relaxed">{assessment.return_feedback}</p>
            </div>
          </AlertDescription>
        </Alert>
      )}
      ...
      {isReadOnly && (
        <Alert className={cn(
          ...
          <AlertDescription className="text-base">
            {isAcknowledged ? "This assessment cycle is complete. Final results have been archived."
              : isAdminReviewed ? "The Admin has released your assessment results. Please review and acknowledge below."
              : isDirectorApprovedOnly ? "Your assessment has been approved by the Director and is currently pending final release by an Administrator."
            ...
            <Button
              ...
              onClick={async () => {
                if (window.confirm("Are you sure you want to delete this assessment? This action cannot be undone.")) {
                  if (await deleteAssessment()) router.push('/dashboard');
                }
              }}
              disabled={saving}
      ...
      <div className="mb-10">
        <Card className="glass-panel border-border/30 shadow-lg overflow-hidden">
          <CardHeader className="pb-4">
            <CardTitle className="text-xl font-bold flex items-center gap-2">
              <Layout className="h-5 w-5 text-primary" />Cycle Progress
            </CardTitle>
          </CardHeader>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <>
              ...
              <Card className="glass-panel border-border/30 overflow-hidden shadow-xl">
                <CardHeader className="bg-emerald-500/5 border-b border-border/10 pb-6 pt-8 px-8">
                  <div className="flex items-center gap-3">
                    <div className="p-2 rounded-lg bg-emerald-500/10 text-emerald-600"><ShieldCheck className="h-6 w-6" /></div>
                ...
                <CardContent className="pt-8 px-8 pb-10">
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-50 border border-emerald-500/20 text-emerald-700">
                      ...
                      <div className="text-sm font-bold">Director Approved & Signed</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="glass-panel border-border/30 overflow-hidden shadow-2xl">
                <CardHeader className="bg-primary/5 border-b border-border/10 pb-6 pt-8 px-8">
                  <div className="flex items-center gap-3">
                    <div className="p-2 rounded-lg bg-primary/10 text-primary"><MessageSquare className="h-6 w-6" /></div>
                ...
                <CardContent className="pt-8 px-8 pb-10">
                  {isAcknowledged ? (
                    <div className="space-y-6">
                      ...
                      <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-50 border border-emerald-500/20 text-emerald-700">
                        <ShieldCheck className="h-6 w-6" />
                        <div className="text-sm font-bold">Electronically Signed & Acknowledged</div>
                      </div>
                    </div>
                  ) : (
                      ...
                      <div className="bg-amber-50 border-l-4 border-amber-400 p-5 rounded-r-xl text-amber-900 shadow-sm">
                        <div className="flex gap-4">
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-sm opacity-80 leading-relaxed">Please review the final results and manager feedback across all domains. Provide your signature and any final reflections below to conclude this cycle.</p>
                          </div>
                        </div>
                      </div>
                      <div className="space-y-4">
              ...
              {isDirectorApprovedOnly && (
                <Card className="glass-panel border-border/30 p-8 text-center flex flex-col items-center justify-center min-h-[400px]">
                  ...
                  <p className="text-muted-foreground max-w-md mx-auto text-lg leading-relaxed">
                    Your assessment has been approved by the Director and is currently awaiting final release by the Administration team. You will be notified once the results are available for your acknowledgment.
                  </p>
                </Card>
              )}
              <Accordion type="multiple" className="w-full space-y-4">
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="sticky top-24 space-y-8">
            {showComparison && (
              <WeightedScoreDisplay domains={domains} score={finalWeightedScore} label="Final Score" type="manager" showAlways={true} />
            )}
            {!showComparison && (
              <WeightedScoreDisplay domains={domains} score={weightedScore} label="Projected Score" type="staff" showAlways={true} />
            )}
              ...
              <CardContent className="pt-6 pb-6 px-6">
                <div className="flex gap-4 items-start">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed italic">
                    The Grade is calculated based on domain weights defined in the organizational playbook. KPIs marked as <strong>&apos;X&apos;</strong> are excluded from the performance calculation for this period.
                  </p>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-t border-border/50 py-4 shadow-2xl animate-in slide-in-from-bottom-full duration-300">
          <div className="container px-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 w-full md:w-auto">
              <Button variant="outline" onClick={saveDraft} disabled={saving} className="flex-1 md:flex-none h-12 px-6 rounded-xl border-primary/20 hover:bg-primary/5 transition-all">
                {saving ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Save className="h-4 w-4 mr-2" />}
                Save Draft
              </Button>
              <Button className="flex-[2] md:flex-none h-12 px-8 rounded-xl font-bold glow-primary transition-all duration-300" onClick={submitAssessment} disabled={saving}>
                <Send className="h-4 w-4 mr-2" />Submit Review
              </Button>
            </div>
          </div>
