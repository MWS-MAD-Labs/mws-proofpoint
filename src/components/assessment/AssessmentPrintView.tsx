// ✅ FIX: merge 2 import dari useAssessment jadi satu
//         Assessment & DomainData hanya dipakai sebagai type → import type
import { calculateWeightedScore, type Assessment, type DomainData } from "@/hooks/useAssessment";
...
function getPerformanceTier(score: number) {
  if (score >= 3.9) return {
    label: "Exemplary", grade: "★", color: "text-blue-600",
    bg: "bg-blue-50", borderColor: "border-blue-200",
    description: "Outstanding performance that exceeds expectations across all domains.",
    bonus: "100%"
  };
  if (score >= 3.6) return {
    label: "Trail Blazers", grade: "◆", color: "text-blue-600",
    bg: "bg-blue-50", borderColor: "border-blue-200",
    description: "High-performing individuals who go beyond role expectations.",
    bonus: "90%"
  };
  if (score >= 3.4) return {
    label: "Rising Star", grade: "▲", color: "text-blue-500",
    bg: "bg-blue-50", borderColor: "border-blue-200",
    description: "Employees showing significant growth and potential.",
    bonus: "80%"
  };
  if (score >= 3.2) return {
    label: "Solid Foundation", grade: "●", color: "text-emerald-600",
    bg: "bg-emerald-50", borderColor: "border-emerald-200",
    description: "Reliably meets role expectations.",
    bonus: "65%"
  };
  if (score >= 3.0) return {
    label: "Developing", grade: "◐", color: "text-emerald-500",
    bg: "bg-emerald-50", borderColor: "border-emerald-200",
    description: "Entry level grade, expected to progress.",
    bonus: "50%"
  };
  if (score >= 2.8) return {
    label: "Needs Improvement", grade: "○", color: "text-amber-600",
    bg: "bg-amber-50", borderColor: "border-amber-200",
    description: "Improvement required within defined period.",
    bonus: "40%"
  };
  if (score >= 2.6) return {
    label: "Performance Management", grade: "!", color: "text-red-600",
    bg: "bg-red-50", borderColor: "border-red-200",
    description: "Immediate intervention required.",
    bonus: "10%"
  };
  return {
    label: "Below Threshold", grade: "—", color: "text-red-700",
    bg: "bg-red-50", borderColor: "border-red-200",
...
export function AssessmentPrintView({ assessment, domains, staffName }: AssessmentPrintViewProps) {
  ...
  const domainScores = domains.map((domain, idx) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const scoredKPIs = allKPIs.filter((k: any) =>
      k.managerScore !== null && k.managerScore !== undefined && k.managerScore !== 'X'
    );
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const avg = scoredKPIs.length > 0
      ? scoredKPIs.reduce((acc: number, k: any) => acc + Number(k.managerScore), 0) / scoredKPIs.length
      : null;
    return {
      ...domain,
      code:     `D${idx + 1}`,
        ...
        <div className="flex justify-between items-center max-w-[210mm] mx-auto">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center overflow-hidden p-0.5">
              <Image src="/millennia-logo.png" alt="Millennia World School" width={36} height={36} className="object-contain" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-12 gap-4">
            <div className={cn("col-span-4 p-3 rounded-lg border", tier.bg, tier.borderColor)}>
              <div className="flex items-baseline gap-1 mb-1">
                <span className={cn("text-2xl font-black tracking-tighter", tier.color)}>{managerScore.toFixed(2)}</span>
                <span className={cn("text-lg", tier.color)}>{tier.grade}</span>
              </div>
              <div className={cn("text-[9px] font-bold uppercase tracking-wide mb-2", tier.color)}>{tier.label}</div>
              <p className="text-[8px] text-slate-600 leading-relaxed mb-2">{tier.description}</p>
        ...
        <section className="space-y-3 pt-2">
          ...
          {domainScores.map((domain, domainIdx) => (
            <div key={domain.id} className="avoid-break">
              <div className="bg-slate-800 text-white px-3 py-1.5 rounded-t-md flex justify-between items-center">
                ...
                {domain.standards.map((standard, stdIdx) => (
                  <div key={standard.id} className="avoid-break">
                    <div className="bg-slate-50 px-3 py-1.5 border-b border-slate-200 flex items-center gap-2">
                      <span className="text-[8px] font-mono font-bold text-slate-400">S{domainIdx + 1}.{stdIdx + 1}</span>
                          ...
                          return (
                            <tr key={kpi.id} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50/50">
                              <td className="py-1.5 pl-6 pr-2 w-12">
                                <span className="font-mono text-[8px] text-slate-400">K{domainIdx + 1}.{stdIdx + 1}.{kpiIdx + 1}</span>
                              </td>
