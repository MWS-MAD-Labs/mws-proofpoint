...
export function ScoreComparisonWidget({
  ...
  const projectedWeighted =
    projectedScore !== undefined && projectedScore !== null
      ? projectedScore
      : calculatedProjected;

  const finalGrade =
    finalWeighted !== null ? getLetterGrade(finalWeighted) : null;
  const projectedGrade =
    projectedWeighted !== null ? getLetterGrade(projectedWeighted) : null;
    ...
    domains!.forEach((d) => {
      d.standards.forEach((s) => {
        s.kpis.forEach((k) => {
          ...
          if (
            k.score === "X" ||
            (k.managerScore !== null && k.managerScore !== undefined)
          ) {
            completedKPIs++;
          }
        });
      ...
      {hasDomains && (
        <div className="px-4 py-3 space-y-1.5">
          ...
          {domainAverages.map((domain, idx) => (
            ...
            >
              <span
                className="text-muted-foreground truncate flex-1 mr-2 text-xs"
                title={domain.name}
              >
                {domain.name}
              </span>
